...
# dataloader for cascaded network -- We changed all functions to work with intermediate outputs, so it's a child of DataLoader
class nnUNetMultitaskCascadeDataLoader(DataLoader):
    def __init__(self,
    ...
    def determine_shapes(self):
        # load one case
        data, seg, _, properties = self._data.load_case(self._data.identifiers[0])
        num_color_channels = data.shape[0]
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        channels_seg = seg.shape[0]
        seg_shape = (self.batch_size, channels_seg, *self.patch_size)
...
# data loader for semi-supervised approach, only major change from cascade is using a float for segmentation type
